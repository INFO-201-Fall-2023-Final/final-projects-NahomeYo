...
ui <- navbarPage(
  ...
  tabPanel("Introduction", 
           fluidPage(
             ...
             p("The primary dataset for this dataset “df”, located in our github repository (https://github.com/INFO-201-Fall-2023-Final/final-projects-NahomeYo/blob/main/df.csv) ,which comprehensively covers the ongoing Israel-Palestine conflict from the past decades, from 2002 to 2020. Data wrangling was employed, specifically focusing on West Bank and Gaza's poverty and casualties rates. The dataset is1716 rows and 9 columns, in which all the data accounts for the Yeas, the region of poverty and casualties, mostly West Bank and Gaza. Indicator/Causes of Poverty, the numerical value associated with the poverty indicators, the month, Number of Palestinians, Number of Israelis killed, The poverty level associated with the indicators and lastly, the The total number of people amounst israelis and palestians.",
               style = "color: #e3f2ff; text-align: left;"),  
  ...
  tabPanel("Poverty Observation",
           fluidPage(
             ...
             sidebarLayout(
               sidebarPanel(
                 sliderInput("year_range_poverty", label = "Select Year Range",
  ...
  tabPanel("Causalities Observation",
           fluidPage(
             ...
             sidebarLayout(
               sidebarPanel(
                 sliderInput("year_range_casualities", label = "Select Year Range",
                             min = min(povertyCasualties$Year),
                             max = max(povertyCasualties$Year),
                             value = c(min(povertyCasualties$Year), max(povertyCasualties$Year)),
                             step = 1),
...
server <- function(input, output) {
  ...
  output$year_table_poverty <- renderTable({
    data_table <- filter(select(povertyCasualties, -Value, -Month, -Palestinians.Killed, -Israelis.Killed, -Total_Killed), Year >= input$year_range_poverty[1] & Year <= input$year_range_poverty[2])
    data_table
  })
  
  output$year_table_casualities <- renderTable({
    data_table <- filter(select(povertyCasualties, -Value, -Month, -Indicator.Name), Year >= input$year_range_casualities[1] & Year <= input$year_range_casualities[2])
    data_table
  })
  
  output$area_plot_poverty <- renderPlot({
    data_plot <- povertyCasualties[povertyCasualties$Year >= input$year_range_poverty[1] & povertyCasualties$Year <= input$year_range_poverty[2], ]
  ...
  output$area_plot_causalities <- renderPlot({
    data_plot <- povertyCasualties[povertyCasualties$Year >= input$year_range_casualities[1] & povertyCasualties$Year <= input$year_range_casualities[2], ]
